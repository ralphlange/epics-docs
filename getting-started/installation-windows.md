...
which makes it very easy to install the MinGW toolchains (32 and 64 bit)
and all other necessary tools.
It also offers a bash shell.
If you are used to working in a Linux environment,
...
either in the same directory as the IOC binary or in a directory
that is mentioned in the `%PATH%` environment variable.
...
The DLL is in memory once and used concurrently by all IOC binaries,
...
Setting the environment for building and running applications
has to be done for either set of choices.

:::{toctree}
:maxdepth: 1

installation-windows-msys2.md
installation-windows-plain.md
installation-windows-env.md
:::
