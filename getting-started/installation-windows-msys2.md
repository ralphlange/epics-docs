...
"mingw-w64-x86_64-" for the 64bit toolchain.
...
dbLoadDatabase("C:\msys64\home\'user'\base-7.0.10\bin\windows-x64-mingw\..\..\dbd\softIoc.dbd")
...
In the same directory, open `Makefile` and change the line `#DB += xxx.db` to:
...
:::{tip}
**Visual clues for success**

When the IOC starts, you should see the EPICS version number and an `epics>`
prompt. You can type `dbl` to see the list of loaded PVs.
:::
